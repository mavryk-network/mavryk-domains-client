export const BuiltInAddresses = {
    mainnet: {
        nameRegistry: {
            address: 'TODO',
            resolveProxyContract: true,
        },
        'nameRegistry:set_child_record': {
            address: 'TODO',
        },
        'nameRegistry:update_record': {
            address: 'TODO',
        },
        'nameRegistry:claim_reverse_record': {
            address: 'TODO',
        },
        'nameRegistry:update_reverse_record': {
            address: 'TODO',
        },
        'tldRegistrar:tez': {
            address: 'TODO',
            resolveProxyContract: true,
        },
        'tldRegistrar:tez:commit': {
            address: 'TODO',
        },
        'tldRegistrar:tez:buy': {
            address: 'TODO',
        },
        'tldRegistrar:tez:renew': {
            address: 'TODO',
        },
        'tldRegistrar:tez:bid': {
            address: 'TODO',
        },
        'tldRegistrar:tez:settle': {
            address: 'TODO',
        },
        'tldRegistrar:tez:withdraw': {
            address: 'TODO',
        },
    },
    edonet: {
        'nameRegistry:set_child_record': {
            address: 'KT1Mq1zd986PxK4C2y9S7UaJkhTBbY15AU32',
        },
        'nameRegistry:update_record': {
            address: 'KT1FpHyP8vUd7p2aq7DLRccUVPixoGVB4fJE',
        },
        'nameRegistry:claim_reverse_record': {
            address: 'KT1EeRLdEPJPFx96tDM1VgRka2V6ZyKV4vRg',
        },
        'nameRegistry:update_reverse_record': {
            address: 'KT1AQmVzLnNWtCmksbCGg7np9dmAU5CKYH72',
        },
        nameRegistry: {
            address: 'KT1KP2Yy6MNkYKkHqroGBZ7KFN5NdNfnUHHv',
            resolveProxyContract: true,
        },
        'tldRegistrar:edo:commit': {
            address: 'KT1KQkkVMTRhGUfJYbHBoaeJ6NUJi8o58cvg',
        },
        'tldRegistrar:edo': {
            address: 'KT1KQkkVMTRhGUfJYbHBoaeJ6NUJi8o58cvg',
            resolveProxyContract: true,
        },
        'tldRegistrar:edo:buy': {
            address: 'KT19SXU8zDaWyiE3WSJgGRJ2xKSsNpFWJoe7',
        },
        'tldRegistrar:edo:renew': {
            address: 'KT1EgSMNJVETfEUGChB1z3xSD4LNVFpkoSyB',
        },
        'tldRegistrar:edo:bid': {
            address: 'KT1VqYtb17QiyBseTshwEAoowyy6FYDHniX1',
        },
        'tldRegistrar:edo:withdraw': {
            address: 'KT1KeuLTd8fXj2G3RgGJAJtU1aVLgp5qcGWQ',
        },
        'tldRegistrar:edo:settle': {
            address: 'KT1FSm8RUYA2maC4mwt37VmFi81ZWZcsZ12w',
        },
        'tldRegistrar:a1:commit': {
            address: 'KT193DkoQdmkf65EZEqSn8rQUxJaL3dVFEon',
        },
        'tldRegistrar:a1': {
            address: 'KT193DkoQdmkf65EZEqSn8rQUxJaL3dVFEon',
            resolveProxyContract: true,
        },
        'tldRegistrar:a1:buy': {
            address: 'KT1XuitgCwwMPYaeYxb7Vr8GARPTwfB16e3G',
        },
        'tldRegistrar:a1:renew': {
            address: 'KT1BQAYkKKh5Hyy4R35SoXb7BKxjtYheb2x6',
        },
        'tldRegistrar:a1:bid': {
            address: 'KT1Kp6z5Zxa9RQU3QqUamfmtQc7sNpnaZftR',
        },
        'tldRegistrar:a1:withdraw': {
            address: 'KT1GRzPpyYAsbgt5D5S8ttTH9YjTKW43WiFv',
        },
        'tldRegistrar:a1:settle': {
            address: 'KT1JwPcaS58sw8jfRhrmbb9YoaLhYrrmjMEs',
        },
        'tldRegistrar:a2:commit': {
            address: 'KT19TYHsHfVZR2Z5Wy5xACfReY99CxszCDRE',
        },
        'tldRegistrar:a2': {
            address: 'KT19TYHsHfVZR2Z5Wy5xACfReY99CxszCDRE',
            resolveProxyContract: true,
        },
        'tldRegistrar:a2:buy': {
            address: 'KT1JBijPG3YdJCwn4tuFvo1wh4bWGWjXXj3b',
        },
        'tldRegistrar:a2:renew': {
            address: 'KT1DCuyTBo1Y4LBjt8Nvouz8Lxphh3qmQHY1',
        },
        'tldRegistrar:a2:bid': {
            address: 'KT1AYiVUHrYrUqN1X91tPgLV3th8N3Khs1sf',
        },
        'tldRegistrar:a2:withdraw': {
            address: 'KT1PMmiLd5vAuWgAp8kDt1GsSYjd2sotpdvE',
        },
        'tldRegistrar:a2:settle': {
            address: 'KT19XV8SZpkujYYJs187YFYwuswb3pXzdjsA',
        },
        'tldRegistrar:a3:commit': {
            address: 'KT1SNCVpRYchsxjisZfQGheSa5yN3fikU6nb',
        },
        'tldRegistrar:a3': {
            address: 'KT1SNCVpRYchsxjisZfQGheSa5yN3fikU6nb',
            resolveProxyContract: true,
        },
        'tldRegistrar:a3:buy': {
            address: 'KT1MibqoywY45wG2m5UTg27P5vPGd77CtvxF',
        },
        'tldRegistrar:a3:renew': {
            address: 'KT1FdGqxVfStgZCQjmm1V6sZFjRNqtXrBfNN',
        },
        'tldRegistrar:a3:bid': {
            address: 'KT1GsDXUjV7NfbD8Yy41TfgLdRCBdzVx5Zu1',
        },
        'tldRegistrar:a3:withdraw': {
            address: 'KT1LeUX8amwHxCkf13MQPtKxKPWWfjEzWsG8',
        },
        'tldRegistrar:a3:settle': {
            address: 'KT1P8D9A6CSdt3bhbzZMGv6Qj5nMqiCT87tj',
        },
    },
    florencenet: {
        'nameRegistry:set_child_record': {
            address: 'KT1UG9DUYqtCF7XUqjH4QNGF2naoh7D1mscR',
        },
        'nameRegistry:update_record': {
            address: 'KT1GZQLAbp9ekSHutKzaoo6yW5AofhYPRQ3q',
        },
        'nameRegistry:claim_reverse_record': {
            address: 'KT1GJFHpra47WtwZH9uVsjwLPqgfL7LDQJhp',
        },
        'nameRegistry:update_reverse_record': {
            address: 'KT1X21jVp6ooHAkPCj3zvCWyNoqQ312dPFid',
        },
        nameRegistry: {
            address: 'KT1PCx2zUqbaCtg8UcyHn31cMD9yaMHLvL6N',
            resolveProxyContract: true,
        },
        'tldRegistrar:flo:commit': {
            address: 'KT1GTSgwuuB5c2ceGhqhTk4nedZznXYFzh1J',
        },
        'tldRegistrar:flo': {
            address: 'KT1GTSgwuuB5c2ceGhqhTk4nedZznXYFzh1J',
            resolveProxyContract: true,
        },
        'tldRegistrar:flo:buy': {
            address: 'KT1E8XRnTzrF49Z33wuB5WzEurPGrN1ASb1w',
        },
        'tldRegistrar:flo:renew': {
            address: 'KT1DFMXRc57XmK1YCZnYtYM7ZZCZ1CTCF7Q3',
        },
        'tldRegistrar:flo:bid': {
            address: 'KT1W89CnCQxAwaZRnRefyAnBU7rUBP8s6fez',
        },
        'tldRegistrar:flo:withdraw': {
            address: 'KT1AheQhiqZRW3SGX7XiRJZgZzGfNTQNV4nQ',
        },
        'tldRegistrar:flo:settle': {
            address: 'KT1SmSRa18zpw5G514LJBkZm8ynAxrKpp5Tg',
        },
        'tldRegistrar:a1:commit': {
            address: 'KT1JP6eYx6Z53SW8TCrKZCcZno61CJUjWvWp',
        },
        'tldRegistrar:a1': {
            address: 'KT1JP6eYx6Z53SW8TCrKZCcZno61CJUjWvWp',
            resolveProxyContract: true,
        },
        'tldRegistrar:a1:buy': {
            address: 'KT1NJbBbczKYNF3CWn9pXwLjhSg99jUG2Fjd',
        },
        'tldRegistrar:a1:renew': {
            address: 'KT1FPh3iejT98uTsxH1fSz3WPGnMmMjLpwDp',
        },
        'tldRegistrar:a1:bid': {
            address: 'KT1NyigaEvn3Xpmnp567H7De1hqaTAo9drNh',
        },
        'tldRegistrar:a1:withdraw': {
            address: 'KT1VbTTGzABsZez3YHmZo2NnAmjZf6bfddwJ',
        },
        'tldRegistrar:a1:settle': {
            address: 'KT1PZfDZ5JiTmxKuxL9hteXafyjUY6evrFwX',
        },
        'tldRegistrar:a2:commit': {
            address: 'KT1XiyeQzUWPpo1x8AYGQZA4wEDhVAfkKg99',
        },
        'tldRegistrar:a2': {
            address: 'KT1XiyeQzUWPpo1x8AYGQZA4wEDhVAfkKg99',
            resolveProxyContract: true,
        },
        'tldRegistrar:a2:buy': {
            address: 'KT1AgkojjS56AF4n1Kg8ueWyUK1uNWncjRrE',
        },
        'tldRegistrar:a2:renew': {
            address: 'KT1WQ4S1UamDy1g5UBLvozSgprStrq6i3xrx',
        },
        'tldRegistrar:a2:bid': {
            address: 'KT1RfaxpqJ2rGD3umeS85N4e7PGGibA5Qcgg',
        },
        'tldRegistrar:a2:withdraw': {
            address: 'KT1FQjUog81bN1ixL6U3X5EWzrD87qkCic3y',
        },
        'tldRegistrar:a2:settle': {
            address: 'KT1LdvFBCftEZ1fkwBseXkxx8XfAiCmqtRuE',
        },
        'tldRegistrar:a3:commit': {
            address: 'KT1MiAUbR16Sz3vWpp8HYdiiybGeLvzH6inP',
        },
        'tldRegistrar:a3': {
            address: 'KT1MiAUbR16Sz3vWpp8HYdiiybGeLvzH6inP',
            resolveProxyContract: true,
        },
        'tldRegistrar:a3:buy': {
            address: 'KT18p1t2fyJTDALqzimiMQe7gAGHeFbc9PRt',
        },
        'tldRegistrar:a3:renew': {
            address: 'KT1Cx3iRQ2xZsDJ62FZ6hd4B8o9g4jq93N3J',
        },
        'tldRegistrar:a3:bid': {
            address: 'KT19WncaVpfnNTFnKDKJuztXWjvVjo198wKm',
        },
        'tldRegistrar:a3:withdraw': {
            address: 'KT19DyPDFYUbJa3EHKv8jz5Wsa36QWNVWY3k',
        },
        'tldRegistrar:a3:settle': {
            address: 'KT1MHCnvYGxLcqFyDFbdAXSGZUKrbREWQbdV',
        },
    },
    custom: null,
};
