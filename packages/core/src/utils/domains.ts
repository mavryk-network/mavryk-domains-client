/** Get the tld part of the domain name. (i.e. `mav` if domain name is `bob.alice.mav`) */
export function getTld(name: string): string {
    const tokens = tokenizeDomainName(name);
    return tokens[tokens.length - 1];
}

/** Get the label part of the domain name. (i.e. `bob` if domain name is `bob.alice.mav`) */
export function getLabel(name: string): string {
    const tokens = tokenizeDomainName(name);
    return tokens.length > 1 ? tokens[0] : '';
}

/** Get the parent part of the domain name. (i.e. `alice.mav` if domain name is `bob.alice.mav`) */
export function getParent(name: string): string {
    return tokenizeDomainName(name).slice(1).join('.');
}

/** Get the domain name without the tld. (i.e. `bob.alice` if domain name is `bob.alice.mav`) */
export function stripTld(name: string): string {
    const tokens = tokenizeDomainName(name);
    return tokens.slice(0, tokens.length - 1).join('.');
}

/** Get the domain level (i.e. 3, if domain name is `bob.alice.mav`) */
